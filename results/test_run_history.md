...
**Pattern observed: Linear scaling confirmed for the 10th consecutive run; all configs pass cleanly; both 2w and 4w hit the breakpoint ceiling**

Ten consecutive runs now confirm linear scaling in the stress test without exception. Stress ordering: 4w (129ms p95 / 254 RPS) > 2w (243ms / 233 RPS) > 1w (749ms / 173 RPS). After Run 9's 1w stress FAIL (genuine CPU saturation), Run 10 returns 1w to the normal range (749ms — within the typical 740–886ms band). All 10 tests pass across all 3 configs — the cleanest possible outcome. Most notable finding: 2w and 4w both completed the breakpoint test in exactly 20 minutes (1,200s) with 0% errors and ~189 RPS — 2w achieved its cleanest breakpoint performance ever, essentially matching 4w's ceiling. 4w still leads on latency (125ms vs 147ms) but throughput is identical at the system ceiling. Recovery produced a clear linear ordering for the seventh consecutive run: 4w (154ms) < 2w (268ms) < 1w (866ms), all 0% errors.

| Test | 1w p95 | 1w RPS | 1w Err | 2w p95 | 2w RPS | 2w Err | 4w p95 | 4w RPS | 4w Err |
|------|--------|--------|--------|--------|--------|--------|--------|--------|--------|
| Stress | 749ms | 173 | 0% | 243ms | 233 | 0% | 129ms | 254 | 0% |
| Breakpoint | 968ms | 72 | 3.43% | 147ms | 189 | 0% | 125ms | 189 | 0% |
| Spike | 825ms | 68 | 0% | 301ms | 102 | 0% | 156ms | 117 | 0% |
| Recovery | 866ms | 74 | 0% | 268ms | 96 | 0% | 154ms | 103 | 0% |

---

## Run 11 — 2026-04-23

**Pattern observed: Linear scaling confirmed for 11th consecutive run; 1w stress FAIL (3rd); 2w and 4w both hit breakpoint ceiling for second consecutive run; 4w recovery anomaly**

Eleven consecutive runs now confirm linear scaling in the stress test without exception. Stress ordering: 4w (136ms p95 / 254 RPS) > 2w (240ms / 234 RPS) > 1w (1,556ms ❌ / 124 RPS). 1w stress exceeded the 1,500ms threshold for the third time across 11 runs (Runs 7, 9, and 11) — confirming that single-worker CPU saturation at 300 VUs is a recurring risk. 2w and 4w remain completely clean. Breakpoint: both 2w and 4w hit the system ceiling for the second consecutive run (189 RPS / 0% errors / 20 min), reinforcing that the ~189 RPS ceiling is a shared infrastructure constraint. 1w had a catastrophic collapse (31,820ms p95 / 47 RPS / 4.82% errors). Spike showed clear linear ordering (4w 165ms < 2w 287ms < 1w 1,820ms). Recovery showed a 4w anomaly: 4w (592ms) was slower than 2w (258ms) — consistent with occasional burst-test variance also seen in Runs 2 and 4 (539ms and 562ms).

| Test | 1w p95 | 1w RPS | 1w Err | 2w p95 | 2w RPS | 2w Err | 4w p95 | 4w RPS | 4w Err |
|------|--------|--------|--------|--------|--------|--------|--------|--------|--------|
| Stress | 1,556ms ❌ | 124 | 0% | 240ms | 234 | 0% | 136ms | 254 | 0% |
| Breakpoint | 31,820ms | 47 | 4.82% | 68ms | 189 | 0% | 104ms | 189 | 0% |
| Spike | 1,820ms | 45 | 0% | 287ms | 103 | 0% | 165ms | 117 | 0% |
| Recovery | 1,804ms | 62 | 0% | 258ms | 96 | 0% | 592ms | 86 | 0% |
...
3. **Linear scaling is the consistent pattern** — All eleven runs confirm 4w as the best config under high load, with 2w in the middle and 1w worst. The stress test (most reliable measurement) shows this ordering without exception across all 11 runs. Run 1's U-curve was a connection pool anomaly; Run 7's anomalies were caused by WSL2 memory fragmentation; Runs 9 and 11's 1w stress FAILs are genuine CPU saturation (2w and 4w were clean in both). Only 4w can reliably reach the system's breakpoint ceiling (~189 RPS) every run — 1w never can; 2w reaches it only when Docker scheduling is favorable (Runs 4, 6, 8, 10, and 11) and collapses or degrades otherwise (Runs 1, 7, 2–3, 5, 9).
4. **4w breakpoint ceiling is rock-solid** — 189 RPS is consistent across all eleven runs (106ms / 112ms / 65ms / 139ms / 50ms / 51ms / 85ms / 167ms / 96ms / 125ms / 104ms). While p95 latency varies with Docker scheduling, the throughput ceiling never moves. This is the most reliable single data point in the entire dataset. In Runs 10 and 11 (two consecutive runs), both 2w and 4w completed the breakpoint in exactly 20 minutes at 189 RPS and 0% errors — the system throughput ceiling is reachable by 2 workers when conditions are favorable.
5. **1w and 2w breakpoint behavior shows high variability across 11 runs** — 1w has shown three regimes: stable ceiling ~1,464–1,483ms (Runs 2–3), moderate collapse with ~47–73 RPS and ~3.4–4.82% errors (Runs 5–6, 8, 9, 10, and 11), and catastrophic collapse to 30,000ms+ (Runs 4 and 7, both caused by Docker scheduling anomalies). Run 11 is a catastrophic collapse. 2w is also volatile: clean ceiling (Runs 4, 6, 8, 10, and 11) vs. degraded (Runs 2–3 and 9) vs. outright collapse (Runs 1 and 7). Both configs are at or near their capacity boundary under open-model arrival rate; small host-load changes produce very different outcomes. Only 4w is structurally immune to collapse in breakpoint.
6. **Burst tests (spike, recovery) remain the most variable** — 4w recovery ranged from 128ms (Run 3) → 539ms (Run 2) → 562ms (Run 4) → 142ms (Run 5) → 146ms (Run 6) → 135ms (Run 7) → 135ms (Run 8) → 124ms (Run 9) → 154ms (Run 10) → 592ms (Run 11). Runs 5–10 all produced a clear recovery ordering: 4w < 2w < 1w. Run 11 showed a 4w recovery anomaly (592ms vs 2w's 258ms) — consistent with Docker CPU scheduling variance also seen in Runs 2 and 4. The consistent finding is that 1w always has the worst spike and recovery latency; 4w is typically fastest but occasionally elevated.
